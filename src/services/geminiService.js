...
const APP_STORAGE_KEY = 'lifeos-app-state-v1'
...
export function getGeminiApiKey() {
  const directKey = localStorage.getItem(GEMINI_KEY_STORAGE)?.trim()
  if (directKey) return directKey

  const appState = safeJsonParse(localStorage.getItem(APP_STORAGE_KEY), {})
  return appState?.settings?.preferences?.geminiApiKey?.trim?.() || ''
}
...
export function saveGeminiApiKey(apiKey) {
  ...
  const appState = safeJsonParse(localStorage.getItem(APP_STORAGE_KEY), {})
  if (appState?.settings?.preferences) {
    appState.settings.preferences.geminiApiKey = cleaned

...
export default function Study() {
  ...
  const [form, setForm] = useState({
    subject: 'Machine Learning', topic: '', focusType: 'Deep Focus',
  ...
  function handleAdd() {
    ...
    setForm({ subject: 'Machine Learning', topic: '', focusType: 'Deep Focus', durationMinutes: 60, date: today, notes: '', rating: 3, pagesRead: 0, problemsSolved: 0, understood: true })
  }
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          ...
          <Card>
            ...
            {todaySessions.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {todaySessions.map(s => <SessionRow key={s.id} session={s} onDelete={handleDelete} />)}
              </div>
            )}
          </Card>
        </>}
          ...
          <Card>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '400px', overflowY: 'auto' }}>
              {[...sessions].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 20).map(s => (
                <SessionRow key={s.id} session={s} onDelete={handleDelete} showDate />
              ))}
            </div>
          </Card>
        </>}
      ...
      {/* ══ LOG SESSION MODAL ══════════════════════════════════ */}
      <Modal isOpen={showAddModal} onClose={() => setShowAddModal(false)} title="Log Study Session">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            ...
            <div>
              ...
              <input style={inputStyle} type="number" placeholder="60" value={form.durationMinutes}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            <div>
              ...
              <input style={inputStyle} type="number" placeholder="0" value={form.pagesRead}
                onChange={e => setForm(f => ({ ...f, pagesRead: e.target.value }))} />
            </div>
            <div>
              <label style={labelStyle}>Problems Solved</label>
              <input style={inputStyle} type="number" placeholder="0" value={form.problemsSolved}
          ...
          <Button onClick={handleAdd} disabled={!form.durationMinutes}>Save Session</Button>
        </div>
      </Modal>
...
// ── Session Row ───────────────────────────────────────────
function SessionRow({ session: s, onDelete, showDate = false }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', borderRadius: '10px', background: 'var(--bg-secondary)', border: '1px solid var(--border)' }}>
      ...
      <button onClick={() => onDelete(s.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: '4px' }}

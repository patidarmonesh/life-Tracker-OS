...
export default function Finance() {
  ...
  const geminiApiKey = state.settings?.preferences?.geminiApiKey || getGeminiApiKey() || '' 
  ...
  function handleAdd() {
    ...
    const newExpense = {
      ...
      isRecurring: false,
      tags: [],
      billImageBase64: linkedBill?.base64 || null,
    ...
    if (linkedBill) {
      updatedBills = bills.map(b =>
        b.id === linkedBill.id
          ? { ...b, linkedExpenseId: newExpense.id }
          : b
      )
    }
  ...
  return (
    ...
    >
      ...
      <button
        ...
        onMouseEnter={evt => {
          evt.currentTarget.style.color = '#F43F5E'
        }}
        onMouseLeave={evt => {
          evt.currentTarget.style.color = 'var(--text-muted)'
        }}
      >
        <Trash2 size={14} />
      </button>
